...
def extract_constructor_dataset(p, simple_summary_closure):
    ...
    if random.random() < base_prob:
        if methodComment.strip():
            prompt = (
                "### Task: Test Class Constructor Summarization\n"
                "Summarize the following constructor in java unit test class using its name, comment.\n\n"
                f"### Constructor Name: {methodName}\n"
                f"### Constructor Snippet: {snippet}"
            )
    ...
    if random.random() < base_prob:
        prompt = (
            "### Task: Test Class Constructor Snippet Generation\n"
            "Generate a Java constructor implementation in java unit test class given its name, signature, and class name.\n\n"
            f"### Class Name: {className}\n"
            f"### Signature: {methodSignature}"
        )
        if methodComment:
            prompt+=f"### methodComment: {methodComment}"
        output = snippet
